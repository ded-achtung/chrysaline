...
TEXTS = [
    # Природоведение
    "Кот живёт в доме. Собака живёт в доме. Корова живёт на ферме. Кот ест рыбу. Собака ест мясо. Корова ест траву. Корова даёт молоко. Кот это млекопитающее. Собака это млекопитающее. Корова это млекопитающее. Ворона это птица. Воробей это птица. Птицы умеют летать. Рыба живёт в воде. Рыба дышит жабрами.",
    # Математика
    "Два плюс три равно пять. Три плюс два равно пять. Один плюс один равно два. Пять минус два равно три.",
    # Русский язык
    "Гласные буквы это а о у и е ы. Жи пишется с буквой и. Ши пишется с буквой и. Предложение начинается с большой буквы. В конце предложения ставится точка.",
    # Литература
    "Мама мыла раму. Папа читал книгу. Бабушка варила кашу.",
    # Сказка
    "Жили-были дед да баба. И была у них курочка Ряба. Снесла курочка яичко. Не простое а золотое. Дед бил бил не разбил. Баба била била не разбила. Мышка бежала хвостиком махнула. Яичко упало и разбилось. Плачет дед плачет баба. А курочка кудахчет.",
    # Репка
    "Посадил дед репку. Выросла репка большая пребольшая. Стал дед репку тянуть. Тянет потянет вытянуть не может. Позвал дед бабку.",
    # Учебник: части речи
    "Существительное это самостоятельная часть речи которая обозначает предмет. Глагол это самостоятельная часть речи которая обозначает действие. Прилагательное это самостоятельная часть речи которая обозначает признак. Предлог это часть речи. Местоимение это часть речи.",
    # Учебник (сжато)
    "Подлежащее это главный член предложения. Сказуемое это главный член предложения. Корень это общая часть слов. Приставка это часть слова. Суффикс это часть слова.",
]
...
def main():
    ...
    while True:
        ...
        if now - last_snapshot >= snapshot_interval:
            ...
            # Считаем живые цепочки
            chains = sum(1 for c in world.creatures.values()
                         if c.alive and "через" in c.parts and "значит" in c.parts)
            ...
            print(f"  [{minutes:02d}:{seconds:02d}] тиков={total_ticks:5d} "
                  f"живых={alive:4d} абстр={abst:3d} цепочек={chains:2d} "
    ...
    for c in world.creatures.values():
        if c.alive and "через" in c.parts and "значит" in c.parts:
            chain_count += 1
            print(f"    {c.name:50s} e={c.energy:.1f} fed={c.times_fed}")
    if chain_count == 0:
        print(f"    (нет живых цепочек)")
    print(f"    Всего: {chain_count}")
