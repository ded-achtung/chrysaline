...
class World:
    ...
    # ═══════════════════════════════════════
    # ГЕНЕРАЦИЯ: развернуть абстракцию в конкретные предложения
    # ═══════════════════════════════════════
    ...
    # ═══════════════════════════════════════
    # ВОПРОСЫ: "что ест кот?" → visiting → ответ
    # ═══════════════════════════════════════

    _Q_WORDS = {"что", "кто", "где", "как", "какой", "какая", "какое",
    ...
    def ask(self, question):
        ...
        q_words = [w for w in words if w in self._Q_WORDS]
        content_words = [w for w in words if w not in self._Q_WORDS
                         and len(w) > 1]
        ...
        known_words = [w for w in content_words
                       if self._find_by_parts((w,)) is not None]
        unknown_words = [w for w in content_words if w not in known_words]

        search_words = known_words if known_words else content_words

        # Стратегия 1: ищем абстракции где ключевые слова = фиксированные части,
        # а ответ = содержимое слотов
        ...
        for word in search_words:
            for c in self.creatures.values():
                ...
                if match_count < min_match:
                    continue
                for slot_name, options in c.slot_options.items():
                    clean = sorted(o for o in options if not o.startswith("$"))
                    if clean:
        ...
        # ищем пересечение с другими ключевыми словами
        if not result["answers"] and len(search_words) >= 2:
            primary = search_words[0]
            info = self.visit(primary)
            if info["found"]:
        ...
        # Стратегия 4: для вопросов типа "что делает X" / "что ест X" —
        # найти организмы где X + глагол → посмотреть остальные части
        if not result["answers"]:
            for word in search_words:
                info = self.visit(word)
                if not info["found"]:
